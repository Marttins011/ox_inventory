...
const WeightBar: React.FC<{ percent: number; weight?: number; maxWeight?: number; durability?: boolean }> = ({ percent, durability, weight, maxWeight }) => {
  const color = useMemo(
    () =>
      durability
        ? percent < 50
  ...
  return (
    <div className={durability ? 'durability-bar' : 'weight-bar'}>
      ...
      >
        {!durability && (<p style={{ padding: 2 }}>
          {(weight !== undefined ? weight : 0) / 1000}/{(maxWeight !== undefined ? maxWeight : 0) / 1000}kg 
        </p>)}
      </div>
    </div>
  );
